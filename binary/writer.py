from binary.defines import WATags, WASingleByteTokens
from utils.utils import getNumValidKeys, encodeUTF8, ceil
...
class WABinaryWriter:
    ...
    def pushIntN(self, value, n, littleEndian=None):
        for i in range(n):
    ...
    def write_node(self, node):
        ...
        num_attributes = getNumValidKeys(node[1]) if bool(node[1]) else 0

        self.writeListStart(2 * num_attributes + 1 + (1 if bool(node[2]) else 0))
        self.write_node_string(node[0])
        self.write_node_attributes(node[1])
        self.write_node_children(node[2])

    def write_node_string(self, token, i=None):
        # Convert json encoded string to str type
    ...
    def writeJid(self, jidLeft, jidRight):
        ...
        if jidLeft is not None and len(jidLeft) > 0:
            self.write_node_string(jidLeft)
        else:
            self.writeToken(WATags.LIST_EMPTY)
        self.write_node_string(jidRight)
    ...
    def write_node_attributes(self, attrs):
        if attrs is None:
            return
        for key, value in attrs.iteritems():
            if value is not None:
                self.write_node_string(key)
                self.write_node_string(value)

    def write_node_children(self, children):
        if children is None:
            return

        if isinstance(children, str):
            self.write_node_string(children, True)
